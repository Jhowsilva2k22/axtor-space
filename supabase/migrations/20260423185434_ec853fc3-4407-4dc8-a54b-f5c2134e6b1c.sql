-- 1) Atualiza create_tenant_for_user para criar uma categoria "Meus links" inicial
...
BEGIN
  ...
  -- Categoria padrão para o novo tenant (evita estado vazio no Select de blocos)
...
-- 2) Backfill: cria categoria "Meus links" para tenants ativos que ainda não têm nenhuma categoria
INSERT INTO public.bio_categories (tenant_id, name, slug, icon, position, is_active)
SELECT t.id, 'Meus links', 'meus-links', 'Link2', 0, true
FROM public.tenants t
WHERE t.status = 'active'
  AND NOT EXISTS (
    SELECT 1 FROM public.bio_categories c WHERE c.tenant_id = t.id
  );
