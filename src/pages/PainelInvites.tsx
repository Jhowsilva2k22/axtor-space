...
const PainelInvites = () => {
  ...
  const sendInviteEmail = async (inv: InviteRow) => {
    ...
    const { error } = await supabase.functions.invoke("send-transactional-email", {
      body: {
        ...
        templateData: { inviteUrl, ...inviterData },
      },
    });
    if (error) {
  ...
  const create = async (e: FormEvent) => {
    ...
    if (recipient) {
      ...
      if (ids.length > 0) {
        await supabase
          .from("invite_codes" as any)
          .update({ revoked_at: new Date().toISOString(), note: `superseded → ${code}` } as any)
          .in("id", ids);
    ...
    const { data: inserted, error } = await supabase
      .from("invite_codes" as any)
      .insert({ code, type, mode, target_email: recipient, note: note.trim() || null, created_by: user.id } as any)
      .select()
      .single();
    setSubmitting(false);
  ...
  const rotateInvite = async (inv: InviteRow) => {
    ...
    const { data: inserted, error: insErr } = await supabase
      .from("invite_codes" as any)
      .insert({ code: newCode, type: inv.type, mode: inv.mode, target_email: inv.target_email, note: inv.note, created_by: user.id } as any)
      .select()
      .single();
    ...
    await navigator.clipboard.writeText(buildLink(newInv));
    ...
    <div className="relative isolate min-h-screen bg-background grain">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-6 py-10">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <Link to="/painel" className="mb-3 inline-flex items-center gap-1 text-xs uppercase tracking-[0.2em] text-muted-foreground hover:text-primary">
